...
/--
Reduce `projName (ctorName a)` to `a`, where `projName`/`ctorName` are the constructor/projector
pair auto-generated by the `newtype` command for some virtual type (see `VirtualStructureInfo`).
This mirrors `reduceProj?`, but for virtual (`def`-based) structures rather than real ones:
`ctorName`/`projName` have no `Expr.proj`/constructor application to pattern-match on, so we
