...
instance : MonadFunctor m (OptionT m) := ⟨fun f x => .mk (f x.run)⟩
...
instance (ε : Type u) [MonadExceptOf ε m] : MonadExceptOf ε (OptionT m) where
  ...
  tryCatch x handle := OptionT.mk <| tryCatchThe ε x.run fun e => (handle e).run
