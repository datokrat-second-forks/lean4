...
@[simp] theorem getElemV_toList {_ : Nonempty α} {xs : Vector α n} {i : Nat} (h : i < xs.size) :
    xs.toList｢i｣ = xs｢i｣ := by
  cases xs
  simp [h]
...
@[simp] theorem getElem?_eq_some_getElemV {_ : Nonempty α} {xs : Vector α n} {i : Nat} (h : i < n) :
    xs[i]? = some xs｢i｣ :=
