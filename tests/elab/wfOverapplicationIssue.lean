theorem Array.sizeOf_lt_of_mem' [DecidableEq α] [SizeOf α] {as : Array α} (h : as.contains a) : sizeOf a < sizeOf as := by
  simp [Membership.mem, contains, any, BEq.beq, anyM] at h
  let rec aux (j : Nat) : (anyM.loop (m := Id) (fun b => pure <| decide (a = b)) as as.size (Nat.le_refl ..) j).run = true → sizeOf a < sizeOf as := by
    unfold anyM.loop
    intro h
    split at h
